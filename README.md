...
A research testbed and non-invasive telemetry suite for evaluating multi-turn LLM agent evasion and cognitive discontinuities in human verification challenges.
...
Generate reproducible evaluation cohorts ($N=140$ trials across Human Baseline, Optimized AI, Adversarial Humanized AI, and Latency-Spoofed Bots):
```powershell
...
Instead of relying on single-shot trivia or easily spoofed visual puzzles, Behavioral Whiplash evaluates **transition divergence** across a two-turn ambiguous cognitive honeypot.
...
2. **Cognitive Rigidity:** Non-steered bots exhibit zero epistemic modulation ($\Delta u = 0.0$) and zero interactive keystroke revisions ($\Delta \kappa = 0.0$).
3. **Composite Detector:** A dual-regime rule combining Semantic Whiplash and Biometric Rigidity achieved **100% accuracy (F1 = 1.0)** across $N=140$ benchmark trials.
...
- **Harmless Cognitive Vignette:** The scenario uses an overtly fictional cartoon ailment to prevent real-world medical misinformation.
